...
def visualize_frame_projection(img_L, img_R, frames_se3, P_left, P_right, points_3d, start_3d, center_3d, end_3d):
    ...
    if len(frames_se3) > 0:
        ...
        cv2.circle(overlay_L, tuple(start_pj_l.astype(int).ravel()), 10, (0, 255, 255), -1)  # Yellow
        cv2.circle(overlay_L, tuple(end_pj_l.astype(int).ravel()), 10, (255, 255, 0), -1)  # Cyan
        cv2.circle(overlay_R, tuple(start_pj_r.astype(int).ravel()), 10, (0, 255, 255), -1)
        cv2.circle(overlay_R, tuple(end_pj_r.astype(int).ravel()), 10, (255, 255, 0), -1)
...
def visualize_needle_3d_live(ax, frames_se3, start_3d, end_3d, points_3d, center_3d):
    ...
    # 반원 점
    ax.scatter(points_3d[:, 0], points_3d[:, 1], points_3d[:, 2], c='silver', s=5, label='Arc points')

    # start / end
    ax.scatter(*start_3d, c='y', s=100, marker='o', label='Head')
    ax.scatter(*end_3d, c='c', s=100, marker='o', label='Tip')

    # center
    if center_3d is not None:
        ax.scatter(*center_3d, c='m', s=80, marker='^', label='Center')
    ...
    plt.draw()
    plt.pause(0.001)  # 화면 갱신 (cv2.imshow랑 비슷한 효과)