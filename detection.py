...
class NeedleDetection:
    def __init__(self, camera):
        ...
        self.map1_l = np.load("/home/surglab/NeedleHandover/NeedleDetection/camera_parameters/map1_l.npy")
        self.map2_l = np.load("/home/surglab/NeedleHandover/NeedleDetection/camera_parameters/map2_l.npy")
        self.map1_r = np.load("/home/surglab/NeedleHandover/NeedleDetection/camera_parameters/map1_r.npy")
        self.map2_r = np.load("/home/surglab/NeedleHandover/NeedleDetection/camera_parameters/map2_r.npy")
        ...
        self.P_L = np.load("/home/surglab/NeedleHandover/NeedleDetection/camera_parameters/P_L.npy")
        self.P_R = np.load("/home/surglab/NeedleHandover/NeedleDetection/camera_parameters/P_R.npy")
    ...
    def get_image(self, which='L'):
        if which == 'L':
            self.image_L = self.camera.get_image(which=which)
            self.image_L = self.rectify(self.image_L, which=which)
            return self.image_L
        elif which == 'R':
            self.image_R = self.camera.get_image(which=which)
    ...
    def get_keypoints(self):
        ...
        # 4
        samples_L, samples_R, angles_L, angles_R = sample_ellipse_ordered_dual(ellipse_L, ellipse_R, mask_L, mask_R, num_samples=100)
        ...
        intersection1_L, intersection2_L = find_segment_intersections(samples_L, angles_L, all_indices_L, segment_labels_L, self.rotation_type_L, angle_thresh)
        intersection1_R, intersection2_R = find_segment_intersections(samples_R, angles_R, all_indices_R, segment_labels_R, self.rotation_type_R, angle_thresh)
        ...
        reconstruct_pts_L, head_2d_L, tip_2d_L = reconstruct_ellipse(ellipse_L, intersection1_L, intersection2_L, self.rotation_type_L)
        reconstruct_pts_R, head_2d_R, tip_2d_R = reconstruct_ellipse(ellipse_R, intersection1_R, intersection2_R, self.rotation_type_R)
        reconstruct_pts_L, reconstruct_pts_R = match_pts_again(ellipse_L, ellipse_R, reconstruct_pts_L, reconstruct_pts_R)

        ## Visualize
        visualize_reconstructed_samples(mask_L, ellipse_L, reconstruct_pts_L, head_2d_L, tip_2d_L, intersection1_L, intersection2_L, which="L")
        visualize_reconstructed_samples(mask_R, ellipse_R, reconstruct_pts_R, head_2d_R, tip_2d_R, intersection1_R, intersection2_R, which="R")
    ...
    def get_needle_frame(self):
        key_pts_L, key_pts_R, (cx_L, cy_L), (cx_R, cy_R), (head_2d_L, head_2d_R), (tip_2d_L, tip_2d_R) = self.get_keypoints()
        if key_pts_L is None or key_pts_R is None:
        ...
        try:
            ...
            center_3d = triangulate(np.array([[cx_L], [cy_L]], dtype=np.float32),
                                    np.array([[cx_R], [cy_R]], dtype=np.float32), self.P_L, self.P_R)[:3].squeeze()
        ...
        try:
            ...
            target_radius = 0.024
            center_3d, actual_radius, normal_vector, inliers = circle_ransac(points_3d, plane_normal, tangent, bitangent, plane_point, target_radius)
        except:
            print("ransac failed")

        if center_3d is None or actual_radius is None:
            print("center3d or actual radius is None")
            return None

        points_3d = points_3d[inliers]
        print("len_points_3d, after circle ransac:", len(points_3d))
        print("actual_radius:", actual_radius)
        print("center3d:", center_3d)

        frames_se3 = compute_needle_frames(start_3d, end_3d, points_3d, center_3d, plane_normal, tangent, rotation_type=self.rotation_type_L)
