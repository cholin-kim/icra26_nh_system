...
def keypoint_segmentation(img, color):
    ...
    img_mask = cv2.inRange(hsv, lower, upper) # for original ros_tracking.py (simulation)
...
def keypoint_segmentation_centroid(img, color, visualize=False):
    ...
    if visualize:
        for c in centroids:
            cv2.circle(img, c, radius=10, color=(0, 0, 255), thickness=2)
...
def detect_shaft_with_hough(image, hsv_min=(0, 0, 0), hsv_max=(180, 255, 182), visualize=False):
    # 1. HSV thresholding으로 shaft 영역 마스킹
    hsv = cv2.cvtColor(image, cv2.COLOR_RGB2HSV)
    mask = cv2.inRange(hsv, hsv_min, hsv_max)
    ...
    # print("Open time:", time.time()-st)
    # st = time.time()
    # mask = fill_holes_in_two_largest_white_regions(mask)
    mask = keep_largest_hole_and_invert(mask)
    # print("fill hoele time:", time.time()-st)
    # st = time.time()
    # 2. Edge detection (Canny)
    edges = cv2.Canny(mask, 50, 150, apertureSize=3)

    # print("Canny time:", time.time()-st)
    # st = time.time()

    # 3. Probabilistic Hough Transform으로 직선 검출
    lines = cv2.HoughLines(edges, rho=1, theta=np.pi / 180, threshold=150)

    # print("hough time:", time.time()-st)
    # st = time.time()
