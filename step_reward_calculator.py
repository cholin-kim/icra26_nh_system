...
class ReachedGoalStrategy(RewardStrategy):
    ...
    def evaluate(self, context: Dict) -> Tuple[bool, float]:
        ...
        # goal_pose = np.array(context['goal_state'][3:], dtype=float)
        # needle_pose = np.array(context['needle_pose'], dtype=float)     # needle pose after the action
        #
        # # (gp, ga, gh), (goal_pose)
        # if np.array_equal(goal_state, state) and np.allclose(goal_pose, needle_pose, rtol=1e-02, atol=1e-02):
