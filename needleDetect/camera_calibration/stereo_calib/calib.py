...
def save_frames_single_camera(camera_name):
    ...
    if camera_name == "camera0":
        ...
        cam = Basler(serial_number="40268300")
    cam.start()
    import time
    while len(cam.image) == 0:
        time.sleep(0.1)
    while True:
        frame = cam.image

        # frame_small = cv.resize(frame, None, fx=1 / view_resize, fy=1 / view_resize)
        frame_small = ImgUtils.resize(frame, 0.5)
...
def save_frames_two_cams(camera0_name, camera1_name):
    ...
    cam_R.start()
    import time
    while len(cam_L.image) == 0 or len(cam_R.image) == 0:
        time.sleep(0.1)
...
def check_calibration(camera0_name, camera0_data, camera1_name, camera1_data, _zshift=50.):
    ...
    while True:
        ...
        frame0 = cam_L.image
        frame1 = cam_R.image
        frame0 = ImgUtils.resize(frame0, 0.5)
        frame1 = ImgUtils.resize(frame1, 0.5)
...
if __name__ == '__main__':
    ...
    cmtx0 = np.array( [[1.78991041e+03, 0.00000000e+00 ,9.83705640e+02],
 [0.00000000e+00, 1.79261133e+03, 5.90762414e+02],
 [0.00000000e+00 ,0.00000000e+00, 1.00000000e+00]])
    dist0 = np.array([[-0.41071445 , 0.29512876, -0.00055565, -0.00188059, -0.22298233]])
    cmtx1 = np.array(  [[1.79776231e+03 ,0.00000000e+00 ,9.75072233e+02],
 [0.00000000e+00 ,1.80042588e+03 ,5.87071215e+02],
 [0.00000000e+00 ,0.00000000e+00 ,1.00000000e+00]]
)
    dist1 = np.array([[-0.44392477  ,0.49302642 ,-0.00106891, -0.0020706 , -0.62513242]]
)
    R, T = stereo_calibrate(cmtx0, dist0, cmtx1, dist1, frames_prefix_c0, frames_prefix_c1)
    print("R\n", R)
    print("T\n", T)
 #    R = np.array( [[ 0.99981503 , 0.00208609  ,0.01911926],
 # [-0.0020354 ,  0.99999436, -0.00267066],
 # [-0.01912472 , 0.00263125 , 0.99981364]])
 #    T = np.array( [[ 4.15992101],
 # [-0.0182169 ],
 # [-0.30972699]])
    #
