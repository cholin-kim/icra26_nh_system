...
class MovingAverage:
    def __init__(self, nb_average, nb_axis):
        self.buffer = np.zeros((nb_average, nb_axis))
...
class LPF:
    def __init__(self, fc, fs, order, nb_axis=1): # fc=cut off freq, fs=sampling rate
        ...
        self.b, self.a = butter(N=order, Wn=normal_cutoff, btype='low', analog=False)    # Get the filter coefficients
